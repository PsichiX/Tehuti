...
use tehuti::{
    ...
    peer::{PeerBuilder, PeerDestructurer, PeerFactory, PeerRoleId, TypedPeer},
};
...
fn client(meeting: MeetingInterface, terminate_sender: Sender<()>) {
    ...
    loop {
        if is_your_turn {
            ...
            peer.make_move_events.send(event).unwrap();
        } else {
            println!("Waiting for opponent's move...");

            match peer.game_state_events.recv_blocking().unwrap() {
                GameStateEvent::Update(state) => {
                    peer.state = state;
                }
                GameStateEvent::Retry => {
                    println!("You've made an invalid move. Try again.");
                    continue;
                }
            }
...
struct GameRole {
    make_move_events: Sender<MakeMoveEvent>,
    game_state_events: Receiver<GameStateEvent>,
    state: GameState,
}
...
impl TypedPeer for GameRole {
    fn builder(builder: PeerBuilder) -> PeerBuilder {
        builder
            .bind_write::<PostcardCodec<MakeMoveEvent>, MakeMoveEvent>(
                MAKE_MOVE_CHANNEL,
                ChannelMode::ReliableOrdered,
                None,
            )
            .bind_read::<PostcardCodec<GameStateEvent>, GameStateEvent>(
                GAME_STATE_CHANNEL,
                ChannelMode::ReliableOrdered,
                None,
            )
    ...
    {
        Ok(Self {
            make_move_events: peer.write(MAKE_MOVE_CHANNEL)?,
            game_state_events: peer.read(GAME_STATE_CHANNEL)?,
