...
impl Codec<Vec<u8>> for RawBytesCodec {
    fn encode(message: &Vec<u8>, buffer: &mut dyn Write) -> Result<(), Box<dyn Error>> {
        buffer.write_all(&(message.len() as u64).to_be_bytes())?;
        buffer.write_all(message)?;
        Ok(())
    }

    fn decode(buffer: &mut dyn Read) -> Result<Vec<u8>, Box<dyn Error>> {
...
pub mod tests {
    ...
    impl<T: Serialize + DeserializeOwned> Codec<T> for PostcardCodec<T> {
        fn encode(message: &T, buffer: &mut dyn Write) -> Result<(), Box<dyn Error>> {
