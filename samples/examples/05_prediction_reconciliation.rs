    ...
    fn server_find_input_divergence(&mut self) -> Result<Option<TimeStamp>, Box<dyn Error>> {
        ...
        for player in self.players.values_mut() {
                ...
                && let Some(Dispatch { message, .. }) = input_receiver.last()
            {
                ...
                match (div, divergence) {
                    (Some(new_div), None) => divergence = Some(new_div),
                    (Some(new_div), Some(old_div)) if new_div < old_div => {
                        divergence = Some(new_div)
                    }
                    _ => {}
                }
            }
            ...
            match player {
                ...
                PlayerRole::ClientLocal {
                    ...
                    let since = TimeStamp::new(
                        current_tick
                            .ticks()
                            .saturating_sub(CLIENT_SEND_INPUT_WINDOW),
                    );
                    if let Some(event) =
